  ...
  <div class="chat-planner">
    ...
    <div class="convo-bar">
      ...
      <div class="convo-actions">
        <span v-if="usageInfo && usageInfo.chat" class="usage-tip">
          今日对话 {{ usageInfo.chat.used }}/{{ usageInfo.chat.limit }} · 规划 {{ usageInfo.plan.used }}/{{ usageInfo.plan.limit }}
        </span>
...
import { chatPlan, learnFromPlan, getMyUsage } from '../api'
...
const usageInfo = ref(null) // 用户调用次数使用情况
...
async function doPersist() {
  ...
  try {
    ...
    try {
      ...
      console.error('[本地会话保存失败]', localErr)
    }
  }
}
...
async function send(text) {
  ...
  try {
    ...
    // 更新使用情况
    if (res.usage_info) {
      usageInfo.value = res.usage_info
    } else {
      refreshUsage().catch(() => {})
    }
...
onMounted(() => {
  ...
  window.addEventListener('travel:user-changed', () => refreshUsage().catch(() => {}))
})
onBeforeUnmount(() => {
  clearTimeout(persistTimer)
  persistNow()
