<template>
  <div class="day-card card">
      ...
      >
        ...
        <div class="body">
          <div class="row1">
            ...
            <button class="ai-explain-btn" @click.stop="openAiExplain(item.poi)" title="AI完整讲解">
              ✨ AI讲解
            </button>
            <button :class="['voice-btn', { playing: playingPoiId === item.poi.id }]" @click.stop="toggleVoice(item.poi)" :title="playingPoiId === item.poi.id ? '停止播放' : '语音介绍'">
              {{ playingPoiId === item.poi.id ? '⏸️' : '🔊' }}
            </button>
          </div>
          <div class="row2">
          ...
          <div v-if="item.transport" class="transit">
            <span class="arrow">↑</span> {{ item.transport }} {{ (item.distance_m / 1000).toFixed(1) }}km ·
            约{{ item.transit_min }}分钟
          </div>
          ...
          <div v-if="item.poi.has_hierarchy && (item.poi.inner_route?.length || item.poi.nearby_attractions?.length)" class="hierarchy-section">
            ...
            <div v-if="expandedPoi === item.poi.id" class="hierarchy-detail">
              ...
              <div v-if="item.poi.inner_route?.length" class="inner-route">
                <div class="section-title">🗺️ 内部游览动线（{{ item.poi.inner_route.length }}站，约{{ totalInnerDuration(item.poi.inner_route) }}分钟）</div>
                <div class="route-flow">
                  <template v-for="(stop, idx) in item.poi.inner_route" :key="idx">
                    <div :class="['route-stop', { 'must-see': stop.must_see }]">
                      <div class="stop-header">
                        <span class="stop-order">{{ stop.order }}</span>
                        <span class="stop-name">{{ stop.name }}</span>
                        <span v-if="stop.must_see" class="must-see-badge">必看</span>
                      </div>
                      ...
                      <div class="stop-meta">
                        <span class="stop-duration">⏱️ 约{{ stop.duration_min }}分钟</span>
                      </div>
                    </div>
                    <div v-if="idx < item.poi.inner_route.length - 1" class="route-arrow">
                      <div class="arrow-line"></div>
                      <div class="arrow-icon">→</div>
                    </div>
                  </template>
                </div>
                <div v-if="item.poi.description" class="poi-desc">
                  <div class="section-title">📖 景点介绍</div>
                  <div class="desc-text">{{ item.poi.description }}</div>
                </div>
                <div v-if="item.poi.best_time" class="poi-best-time">
                  <div class="section-title">🕐 最佳游玩时间</div>
                  <div class="best-time-text">{{ item.poi.best_time }}</div>
                </div>
              </div>
    ...
    <div v-if="day.hotel" class="hotel">
      <span class="hotel-icon">🏨</span> 当晚住宿：{{ day.hotel.name }}
      <span v-if="day.hotel.price" class="hotel-price">¥{{ day.hotel.price }}/晚</span>
    </div>
...
// 生成语音脚本
function generateVoiceScript(poi) {
  const parts = []
  parts.push(`欢迎来到${poi.name}。`)
  if (poi.description) {
    parts.push(poi.description)
  }
  if (poi.poi_level) {
    parts.push(`这里是${poi.poi_level}级景区。`)
  }
  if (poi.inner_route && poi.inner_route.length > 0) {
    parts.push(`景区内部游览动线共${poi.inner_route.length}站，建议游览顺序为：`)
    poi.inner_route.forEach((stop, idx) => {
      parts.push(`第${idx + 1}站，${stop.name}，${stop.description || ''}，建议停留${stop.duration_min}分钟。`)
    })
  }
  if (poi.best_time) {
    parts.push(`最佳游玩时间：${poi.best_time}。`)
  }
...
function toggleVoice(poi) {
  ...
  // 开始新的播放
  const script = generateVoiceScript(poi)
...
function totalInnerDuration(innerRoute) {
  if (!innerRoute || !innerRoute.length) return 0
  return innerRoute.reduce((sum, stop) => sum + (stop.duration_min || 0), 0)
}
...
.route-stop.must-see .stop-order {
  background: #f59e0b;
}
.stop-name {
  font-size: 13px;
