<template>
  <div class="map-wrap">
    <!-- 行程动画播放控制栏 -->
    <div v-if="isPlaying || canPlay" class="playback-bar">
      ...
      <div class="playback-progress">
        ...
        <span class="progress-text">{{ currentPoiName || '点击播放' }}</span>
      </div>
      <select class="speed-select" v-model="playSpeed" @change="onSpeedChange">
    ...
    <div v-if="!useAmap" class="svg-fallback">
      <svg :viewBox="viewBox" preserveAspectRatio="xMidYMid meet" class="svg">
        ...
        <g v-for="p in plotted" :key="p.poi.id">
          <circle
            :cx="p.x" :cy="p.y" :r="selectedId === p.poi.id ? 12 : 7"
          ...
          <!-- 播放时的脉冲动画 -->
          <circle
            v-if="selectedId === p.poi.id && isPlaying"
            :cx="p.x" :cy="p.y" r="12"
            fill="none"
            stroke="#ef4444"
            stroke-width="2"
            opacity="0.6"
          >
            <animate attributeName="r" from="12" to="25" dur="1s" repeatCount="indefinite" />
...
const playSpeed = ref(1000) // 毫秒/点位
const currentPoiIndex = ref(-1)
const currentPoiName = ref('')
let playTimer = null

// 扁平化所有天的点位，用于播放
const allPlayItems = computed(() => {
  const items = []
...
const canPlay = computed(() => allPlayItems.value.length > 1)

const progressPercent = computed(() => {
  if (allPlayItems.value.length === 0) return 0
  return ((currentPoiIndex.value + 1) / allPlayItems.value.length) * 100
})
...
function startPlay() {
  if (allPlayItems.value.length === 0) return
  isPlaying.value = true
  if (currentPoiIndex.value < 0 || currentPoiIndex.value >= allPlayItems.value.length - 1) {
...
function playNext() {
  ...
  if (currentPoiIndex.value >= allPlayItems.value.length) {
    stopPlay()
    return
  }

  const item = allPlayItems.value[currentPoiIndex.value]
  if (item && item.poi) {
    currentPoiName.value = `第${item.day}天 · ${item.poi.name}`
    focusPoi(item.poi)
  }
  ...
  if (currentPoiIndex.value < allPlayItems.value.length) {
    playTimer = setTimeout(playNext, playSpeed.value)
  } else {
    // 播放完成
    setTimeout(() => {
      isPlaying.value = false
      currentPoiName.value = '播放完成'
    }, playSpeed.value)
  }
}

function onSpeedChange() {
  // 速度改变时，如果正在播放，立即应用到下一个点位
  // 不需要特殊处理，playNext会使用新的playSpeed
}
...
defineExpose({ focusPoi, startPlay, pausePlay, stopPlay, togglePlay })
